...
void EXTI9_5_IRQHandler(void)
{
     ...
     if(EXTI_GetITStatus(EXTI_Line7) != RESET)
	{
			...
			UltrasonicWave_Distance=TIM_GetCounter(TIM2)*340/200.0;									 //计算距离&&UltrasonicWave_Distance<150
			
	if(UltrasonicWave_Distance>0)
	{
		//printf("distance:%f cm",UltrasonicWave_Distance);
		/*OLED_ShowString(1,5,"dist:");
		UltrasonicWave_Distance_temp = UltrasonicWave_Distance;
		
		OLED_ShowNum(40,5,UltrasonicWave_Distance_temp,3,16);
		UltrasonicWave_Distance -= UltrasonicWave_Distance_temp;
		UltrasonicWave_Distance *=100;
		UltrasonicWave_Distance_temp = UltrasonicWave_Distance;
		OLED_ShowChar(65,5,'.');
		OLED_ShowNum(70,5,UltrasonicWave_Distance_temp,2,16);*/
		flag_chaosheng = 1;
	}else{
		flag_chaosheng = 0;
	}
