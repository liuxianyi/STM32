...
{	 
	...
	GPIOA->CRH|=0X0000B000;    //PORTA11复用输出PA11
...
void TIM5_Ctrl_Init(u16 arr,u16 psc)
{
  TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	...
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0x01;  //从优先级1级
...
void TIM3_IRQHandler(void)   //TIM3中断
{
	...
	if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET) //检查指定的TIM中断发生与否:TIM 中断源 
		...
		TIM_ClearITPendingBit(TIM3, TIM_IT_Update  );  //清除TIMx的中断待处理位:TIM 中断源 
			...
			//KEY_Scan();
			//printf("\r\n您发送的消息为:\r\n\r\n");
				//USART_SendData(USART1, '1');//向串口1发送数据，数据发送到电脑
				//while(USART_GetFlagStatus(USART1,USART_FLAG_TC)!=SET);//等待发送结束
			//printf("\r\n\r\n");//插入换行
			/*flag++;
			if(flag==10){
				BEEP=!BEEP;
				USART_SendData(USART1, '3');//向串口1发送数据，数据发送到电脑
				while(USART_GetFlagStatus(USART1,USART_FLAG_TC)!=SET);//等待发送结束
				flag=0;
			}*/
...
{  
   ...
   //初始化TIM3
	...
	TIM_OC2PreloadConfig(TIM3, TIM_OCPreload_Enable);  //使能TIM3在CCR2上的预装载寄存器
		//TIM_ARRPreloadConfig(TIM3, ENABLE); //使能TIMx在ARR上的预装载寄存器，输入捕获
